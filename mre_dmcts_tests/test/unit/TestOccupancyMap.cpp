...
protected:
	...
	mre_dmcts::OccupancyMap occMap_;
...
public:
	...
	void checkParams(mre_dmcts::OccupancyMap occMap) {
...
TEST_F(TestOccupancyMap, ConstructFromMessage) {
	...
	mre_dmcts::OccupancyMap newOccMap(msg);
...
TEST_F(TestOccupancyMap, IsOccupied) {
	...
	mre_dmcts::OccupancyMap map(length, resolution, position, frameId, 98);
	EXPECT_TRUE(map.isOccupied(*it));

	map = mre_dmcts::OccupancyMap(length, resolution, position, frameId, 3);
...
TEST_F(TestOccupancyMap, IsFree) {
	...
	map = mre_dmcts::OccupancyMap(length, resolution, position, frameId, 03);
...
TEST_F(TestOccupancyMap, GetFrontier) {
	mre_dmcts::indexList_t frontier = occMap_.getFrontier();
