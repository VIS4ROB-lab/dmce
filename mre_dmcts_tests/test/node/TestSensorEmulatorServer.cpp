...
protected:
	...
	mre_dmcts::SensorEmulatorServer* se_srv_ = nullptr;
	...
	mre_dmcts_msgs::RobotMapUpdate latestUpdate_;
...
public:
		...
		: nodeHandle_(ros::NodeHandle())
	{
		...
		positionPublisher_ =
			nodeHandle_.advertise<mre_dmcts_msgs::RobotPosition>(
				"RobotPosition", 1, true
			);
	}

	void initMapServer() {
		map_srv_ = new mre_dmcts::GroundTruthMapServer(ros::NodeHandle("/groundStation"), 1);
	}

	void initSEServer(const double& timeout = 0.05) {
		se_srv_ = new mre_dmcts::SensorEmulatorServer(nodeHandle_, 1, timeout);
	}

	void publishPosition(double x, double y) {
		mre_dmcts_msgs::RobotPosition message;
		message.x_position = x;
		message.y_position = y;
		positionPublisher_.publish(message);
	}

	void updateCallback(const mre_dmcts_msgs::RobotMapUpdate& msg) {
...
TEST_P(TestSensorEmulatorServer, OnGetMapUpdate_ValuesAreCorrect) {
	...
	mre_dmcts::GroundTruthMap groundTruth(imageFile, resolution, "map");
