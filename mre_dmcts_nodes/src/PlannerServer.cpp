...
namespace mre_dmcts {
		...
		: NodeServer(nh, robotId), timeSinceLastMapCheck_(0)
	{
		...
		planPublisher_ = nodeHandle_.advertise<mre_dmcts_msgs::RobotPlan>("SharedPlans", 10);
	...
	void PlannerServer::publishPlan_(const mre_dmcts_msgs::RobotPlan& planMsg) {
	...
	void PlannerServer::positionCallback_(
		const mre_dmcts_msgs::RobotPosition& msg
	) {
	...
	void PlannerServer::peerPlanCallback_(const mre_dmcts_msgs::RobotPlan& msg) {
		if (explorationFinished_)
			return;
	...
	bool PlannerServer::planServiceCallback_(
		mre_dmcts_msgs::GetPlan::Request& request,
		mre_dmcts_msgs::GetPlan::Response& response)
	{
		if (explorationFinished_) {
	...
	void PlannerServer::clusterCallback_(const mre_dmcts_msgs::FrontierClusters& msg) {
