...
namespace mre_dmcts {
		...
		: NodeServer(nh, robotId), actionClient_("Pathfinding", false)
	{
		...
		globalPlanClient_ = nodeHandle_.serviceClient<mre_dmcts_msgs::GetPlan>(
		...
		failurePublisher_ = nodeHandle_.advertise<mre_dmcts_msgs::NavigationFailureSignal>(
	...
	void NavigationServer::checkAbortConditions_(const ros::Duration& timeStep) {
		...
		if (planTimedOut)
			...
			mre_dmcts_msgs::NavigationFailureSignal failureMsg;
	...
	pos_t NavigationServer::getStartingPosition_(
		mre_dmcts_msgs::GetStartingArea::Response startingArea
	) {
	...
	void NavigationServer::publishRobotPosition_() {
		mre_dmcts_msgs::RobotPosition msg;
	...
	void NavigationServer::tryFetchNewPlan_(bool goalSuccess) {
		...
		mre_dmcts_msgs::GetPlan plan_msg;
	...
	void NavigationServer::pathCallback_(
			const mre_dmcts_msgs::PathfindingFeedbackConstPtr& pathFeedback)
	{
