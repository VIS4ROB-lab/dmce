...
namespace mre_dmcts {
	...
	void LoggingServer::writeFileHeader_() {
		std::stringstream ss;
		ss << "Package version:," << mre_dmcts_nodes_VERSION << std::endl;
	...
	void LoggingServer::positionCallback_(const mre_dmcts_msgs::RobotPosition& msg) {
		lockGuard_t guard(dataMutex_);
	...
	void LoggingServer::navFailureCallback(const mre_dmcts_msgs::NavigationFailureSignal& msg) {
